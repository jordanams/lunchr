...
else
{
	if($responseArray['ACK'] == 'Success')
	{
		...
		var_dump($responseArray);
		$statut = "0";
		$type_paiment = "Carte bleue";
		$generate_commande = add_commandes($_SESSION['id_user'], $_SESSION['restaurant'], $responseArray['PAYMENTINFO_0_ORDERTIME'], $statut, $type_paiment, $_SESSION['commentaire_commande'], $responseArray['PAYMENTINFO_0_TRANSACTIONID'], $_SESSION['date_resa'], $_SESSION['horraire_resa'], $_SESSION['personne_resa'], $totalttc);
		if($generate_commande = true)
		{
			...
			foreach($panier as $k => $products) {
				...
				$update_quantity = update_quantity($products[1], $panier[$k][0]['lp_id']);
	//var_dump($params);
		}

			if($generate_commande2 = true)
			{
				header('location:index.php?module=summary&action=thanks');
			}
