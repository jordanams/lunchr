...
function insert_resto($nom_resto, $adresse_resto, $longitude, $latitude, $pays, $ville, $code_postal, $page_facebook, $website, $lr_tel, $lup_id, $ltc_id, $lr_from_price) {
	...
	try {
		...
		$curseur->bindValue(':ltc_id', $ltc_id, PDO::PARAM_STR);
		$curseur->bindValue(':lr_from_price', $lr_from_price, PDO::PARAM_STR);
...
function type_cuisine() {
	
	global $connexion;
	try {
			$select = $connexion->prepare("SELECT * FROM lunchr_type_cuisine");
...
function select_mail($mail) {
	...
	global $connexion;
	try {
			$select = $connexion->prepare("SELECT COUNT(lup_mail) FROM lunchr_users_pro WHERE lup_mail=:mail");
